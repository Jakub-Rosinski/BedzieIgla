...
- **Map tiles**: keyless OpenStreetMap raster tiles (`{s}.tile.openstreetmap.org`), darkened with a CSS `filter: invert(100%) hue-rotate(180deg) ...` on `.leaflet-tile-pane` only (not the whole map, so the marker/popup/controls keep their real colors). Originally used CARTO's free `dark_all` tiles (`basemaps.cartocdn.com`) — CARTO retired free keyless raster basemaps and now requires a paid API key; unauthenticated requests still return `200` but with the tiles replaced by an "API KEY REQUIRED" watermark, so the map silently degraded to a blank/watermarked box in production with no error anywhere. Whichever tile domain is actually used in `MapaSection.svelte` MUST match the CSP `img-src` entry exactly (`deploy/nginx.conf.template` + `setup-vps.sh`) — a mismatch is CSP-silent (console warning only), not a visible error.
